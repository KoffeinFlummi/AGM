...
// Edited prefixes for compatability in BWA3_Realism by KoffeinFlummi
...
BWA3_Recoil_fnc_firedEH = {
  ...
  if (_weaponType == primaryWeapon player) then {
    ...
    _powerMulti = getNumber (_config >> "bwa3_recoil_shakeMultiplier");
    ...
    // Reduce camshake if the player is rested (bwa3_autorest).
    if (player getVariable ["bwa3_autorest_rested", false] || player getVariable ["bwa3_autorest_hardrested", false]) then {
      _powerMod = _powerMod - 0.10;
    };

    // Reduce camshake if the player is deployed (bwa3_autorest).
    if (player getVariable ["bwa3_autorest_deployed", false]) then {
      _powerMod = _powerMod - 0.10;
    };
  ...
  _lastFired = _unit getVariable ["bwa3_recoil_lastFired", time];
  _burst = _unit getVariable ["bwa3_recoil_burst", 0];

  // Time between rounds
  
  if (time - _lastFired < 0.45) then {
    _burst = _burst + 1;
    _unit setVariable ["bwa3_recoil_burst", _burst, false];
    ...
    if (_burst > _startDisperse) then { 
      ...
      // Decrease max dispersiion if the player is rested (bwa3_autorest).
      if (player getVariable ["bwa3_autorest_rested", false] || player getVariable ["bwa3_autorest_hardrested", false]) then {
        _maxBurst = 25;
      };

      // Decrease max dispersion if the player is deployed (bwa3_autorest).
      if (player getVariable ["bwa3_autorest_deployed", false]) then {
        _maxBurst = 18;
      };
    ...
    if (_burst > 0) then {
      _unit setVariable ["bwa3_recoil_burst", 0, false];
    };
  };

  _unit setVariable ["bwa3_recoil_lastFired", time];
};
