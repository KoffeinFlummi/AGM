...
class AGM_Core_Default_Keys {
  ...
  class openInteractionMenuSelf {
    ...
    conditionUp = "!isNull (findDisplay 1713999)";
    statementUp = "(findDisplay 1713999) closeDisplay 1; if (AGM_Interaction_SelectedButton >= 0) then {_action = AGM_Interaction_fnc_openMenuSelf select AGM_Interaction_SelectedButton; if (call (_action select 2)) then {call (_action select 1)};};";
    exceptions[] = {"AGM_Drag_isNotDragging", "AGM_Medical_canTreat", "AGM_Interaction_isNotEscorting"};
    key = 221;
    shift = 0;
    control = 1;
