...
_fnc_GetActions = {
	...
	for "_i" from 0 to (_count - 1) do {
		_config = _baseConfig >> _parents select _i >> "AGM_SelfActions";
...
for "_index" from 0 to (count _customActions - 1) do {
	...
	if (_showDisabled || {[_player, _object] call _condition}) then {
		_actions pushBack [_displayName, _statement, _condition, _priority, _subMenu, _icon, _tooltip, _conditionShow, _conditionsTooltip, _distance, _hotkey];
	};
};
