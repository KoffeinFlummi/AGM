...
class CfgFunctions {
  class AGM_DisposableLaunchers {
    class AGM_DisposableLaunchers {
      file = "\AGM_DisposableLaunchers\functions";
...
class Extended_Fired_EventHandlers {
  class CAManBase {
    class BWA3_FiredReplaceATWeapon {
      Fired = "if (local (_this select 0) && {getText (configFile >> 'CfgWeapons' >> _this select 1 >> 'AGM_UsedTube') != ''}) then {_this call AGM_DisposableLaunchers_fnc_replaceATWeapon}";
    };
  };
};
...
class Extended_Take_EventHandlers {
  class CAManBase {
    class AGM_TakeLoadedATWeapon {
      Take = "if (getText (configFile >> 'CfgWeapons' >> _this select 2 >> 'AGM_LauncherClass') != '') then {_this call AGM_DisposableLaunchers_fnc_takeLoadedATWeapon}";
    };
  };
};
