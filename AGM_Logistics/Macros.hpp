...
#define MACRO_UNLOAD \
  class AGM_loadItemCarried { \
    ...
    statement = "[_player getVariable 'AGM_carriedItem', AGM_Interaction_Target] call AGM_Logistics_fnc_openLoadUI"; \
...
#define MACRO_RELOAD_MAGS \
  class AGM_reloadMagazines { \
    ...
    distance = 4; \
    condition = "count ([_player, AGM_Interaction_Target] call AGM_Logistics_fnc_getLoadableMagazines) > 0"; \
...
#define MACRO_DRAGABLE \
  class AGM_DragItem { \
    ...
    conditionShow = "[AGM_Interaction_Target, _player] call AGM_Drag_fnc_isDraggable"; \
    statement = "[AGM_Interaction_Target, _player] call AGM_Drag_fnc_dragObject"; \
    showDisabled = 0; \
    priority = 3; \
    icon = "\A3\ui_f\data\igui\cfg\actions\gear_ca.paa"; \
    hotkey = "R"; \
...
#define MACRO_CUTWIRE \
  class AGM_Wirecutter_cut { \
    ...
    distance = 4; \
    condition = "'ToolKit' in items _player && {alive AGM_Interaction_Target}"; \
...
#define MACRO_CARRYJERRYCAN \
  class AGM_CarryJerrycan { \
    ...
    distance = 4; \
    condition = "[_player, AGM_Interaction_Target] call AGM_Drag_fnc_isCarryable"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Drag_fnc_carryJerrycan"; \
    showDisabled = 1; \
    priority = 3; \
    icon = "\A3\ui_f\data\igui\cfg\actions\gear_ca.paa"; \
  }; \
  class AGM_DropJerrycan { \
    ...
    distance = 4; \
    condition = "[_player, AGM_Interaction_Target] call AGM_Drag_fnc_isCarryingObject"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Drag_fnc_dropJerrycan"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 3; \
    icon = "\A3\ui_f\data\igui\cfg\actions\gear_ca.paa"; \
  ...
  class AGM_CheckFuelJerrycan { \
    ...
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_getFuelAmountJerrycan"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 3; \
...
#define MACRO_CHECKFUEL \
  class AGM_CheckFuelSelf { \
    displayName = "$STR_AGM_Resupply_CheckFuelJerryCan"; \
    condition = "(_player getVariable ['AGM_carriedItem', objNull]) isKindOf 'AGM_JerryCan'"; \
    statement = "[_player] call AGM_Resupply_fnc_getFuelAmountJerrycan"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 3; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
...
#define MACRO_REFUEL \
  ...
  class AGM_Refuel { \
    ...
    distance = 4; \
    condition = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_canRefuel"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_refuelVehicle"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 1.245; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
  ...
  class AGM_DrainFuel { \
    ...
    distance = 4; \
    condition = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_canDrainFuel"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_drainFuel"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 1.235; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
  ...
  class AGM_CheckFuel { \
    ...
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_getFuelAmount"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 1.295; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
...
#define MACRO_REFUELCARGO \
  class AGM_DrainFuelCargo { \
    ...
    distance = 4; \
    condition = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_canDrainFuelCargo"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_drainFuelCargo"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 1.22; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
  ...
  class AGM_CheckFuelCargo { \
    ...
    distance = 4; \
    condition = "alive AGM_Interaction_Target && {speed AGM_Interaction_Target == 0}"; \
    statement = "[_player, AGM_Interaction_Target] call AGM_Resupply_fnc_getFuelAmountCargo"; \
    exceptions[] = {"AGM_Drag_isNotDragging"}; \
    showDisabled = 0; \
    priority = 1.24; \
    icon = "\A3\ui_f\data\igui\cfg\actions\refuel_ca.paa"; \
...
#define MACRO_REFUEL_UAV \
  class AGM_RefuelUAV { \
    ...
    distance = 4; \
    condition = "'AGM_UAVBattery' in items _player && {fuel cursorTarget < 1}"; \
    statement = "[cursorTarget, _player] call AGM_UAVs_fnc_refuel"; \
    showDisabled = 0; \
    priority = 1.245; \
...
#define MACRO_PARADROP \
  class AGM_DropCargo { \
    ...
    distance = 4; \
    condition = "getNumber(configFile >> 'CfgVehicles' >> typeOf vehicle _player >> 'AGM_Paradrop') == 1 && {getPosATL vehicle _player select 2 > 60} && {count (vehicle _player getVariable ['AGM_Logistics_loadedItems', []]) > 0}"; \
    conditionShow = "getNumber(configFile >> 'CfgVehicles' >> typeOf vehicle _player >> 'AGM_Paradrop') == 1"; \
