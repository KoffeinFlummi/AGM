...
if (AGM_player call AGM_Drag_fnc_isDraggingObject) then {

  if (AGM_Modifier > 0) then {
  
    _z = (_this * 0.15);
    AGM_Drag_CurrentHeightChange = CLAMP(AGM_Drag_CurrentHeightChange + _z, 0, 1);
    
    if (AGM_Drag_CurrentHeightChange <= 1 AND {AGM_Drag_CurrentHeightChange >= 0}) then {
    
      _handled = true;
      _draggedObject = AGM_player getVariable ["AGM_carriedItem", objNull];
      if (isNull _draggedObject) exitWith {};
      
      private ["_pos", "_max"];
      _pos = getPosATL _draggedObject;
      _max = (AGM_player ModelToWorld [0, 0, 0.75]) select 2;
      _z = (_pos select 2) + _z;
      _pos set [2, CLAMP(_z, 0, _max)];
      
      detach _draggedObject;
      _draggedObject setPosATL _pos;
      _draggedObject attachTo [AGM_player];
