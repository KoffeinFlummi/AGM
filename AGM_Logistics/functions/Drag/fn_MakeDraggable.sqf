...
if ((isNil "_nilTest") OR {_id == -1}) then {
  ...
  _id = [_object, localize "STR_AGM_Drag_StartDrag", 4, {!(AGM_player call AGM_Drag_fnc_isDraggingObject) AND {[AGM_Interaction_Target, AGM_player] call AGM_Drag_fnc_isDraggable}}, {[AGM_Interaction_Target, AGM_player] call AGM_Drag_fnc_dragObject;}, true, 2.2] call AGM_Interaction_fnc_addInteraction;
  _object setVariable ["AGM_dragActionID", _id, true];
  _id = _object getVariable ["AGM_releaseActionID", -1];
  if (_id == -1) then {
    _id = [_object, localize "STR_AGM_Drag_EndDrag", 4, {(AGM_player call AGM_Drag_fnc_isDraggingObject)}, {AGM_player call AGM_Drag_fnc_releaseObject;}, false, 2.1] call AGM_Interaction_fnc_addInteraction;
