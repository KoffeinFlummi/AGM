...
_this spawn {
  ...
  AGM_Fortifications_Setup attachTo [_unit, _unit worldToModel ((ASLtoATL eyePos AGM_player) vectorAdd (positionCameraToWorld [0, 0, 4] vectorDiff positionCameraToWorld [0, 0, 0]))];
  ...
  ["AGM_Fortifications_Placement", "OnEachFrame", {
    if (AGM_Fortifications_placer != AGM_player) exitWith {
      call AGM_Fortifications_fnc_setupCancel;
    };
    AGM_Fortifications_pfeh_running = true;

    _pos = (ASLtoATL eyePos AGM_player) vectorAdd (positionCameraToWorld [0, 0, 4] vectorDiff positionCameraToWorld [0, 0, 0]);
    _pos set [2, 0];

    if (getPosATL AGM_player distance _pos < 2 || {!alive AGM_player} || {AGM_player getVariable ["AGM_isUnconscious", AGM_player]} || {AGM_player != vehicle AGM_player}) exitWith {
      call AGM_Fortifications_fnc_setupCancel;
    };
    ...
    if (AGM_Modifier == 0) then {
      AGM_Fortifications_Setup setDir (AGM_Fortifications_TweakedAngle + getDir AGM_player);
    };
