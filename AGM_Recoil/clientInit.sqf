...
AGM_Recoil_fnc_firedEH = {systemChat "test";
  ...
  if (_weaponType == primaryWeapon player) then {
    ...
    _powerMulti = getNumber (_config >> "agm_recoil_shakeMultiplier");
    ...
    // Reduce camshake if the player is rested (agm_autorest).
    if (player getVariable ["agm_autorest_rested", false] || player getVariable ["agm_autorest_hardrested", false]) then {
      _powerMod = _powerMod - 0.10;
    };

    // Reduce camshake if the player is deployed (agm_autorest).
    if (player getVariable ["agm_autorest_deployed", false]) then {
      _powerMod = _powerMod - 0.10;
    };
  ...
  _lastFired = _unit getVariable ["agm_recoil_lastFired", time];
  ...
  if (time - _lastFired < 0.45) then {
    ...
    if (_burst > _startDisperse) then { 
      ...
      // Decrease max dispersiion if the player is rested (agm_autorest).
      if (player getVariable ["agm_autorest_rested", false] || player getVariable ["agm_autorest_hardrested", false]) then {
        _maxBurst = 25;
      };

      // Decrease max dispersion if the player is deployed (agm_autorest).
      if (player getVariable ["agm_autorest_deployed", false]) then {
        _maxBurst = 18;
      };
