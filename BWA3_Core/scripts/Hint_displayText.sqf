...
_lastHintTime = BWA3_Hint_lastHint select 0;
_lastHintPriority = BWA3_Hint_lastHint select 1;
...
if (_time > _lastHintTime + _delay || {_priority >= _lastHintPriority}) then {
	hintSilent _text;
	if (_sound) then {playSound "BWA3_Sound_Click"};
	BWA3_Hint_lastHint set [0, _time];
	BWA3_Hint_lastHint set [1, _priority];

	sleep _delay;
	if (_time == BWA3_Hint_lastHint select 0) then {hintSilent ""};
};
