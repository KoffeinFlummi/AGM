...
if (_unit getVariable "AGM_Unconscious") then {
  if (_damageNew > 0.9 and !(isNil "AGM_Medical_PreventDeathWhileUnconscious") and {AGM_Medical_PreventDeathWhileUnconscious}) then {
    _damageNew = 0.89;
  };
} else {
  if (_damageNew > 0.9 and !(isNil "AGM_Medical_PreventInstaDeath") and {AGM_Medical_PreventInstaDeath}) then {
    _damageNew = 0.89;
  };
};
...

{
  ...
  if (_unit getVariable "AGM_Unconscious") then {
    if (_damageFinal > 0.9 and !(isNil "AGM_Medical_PreventDeathWhileUnconscious") and {AGM_Medical_PreventDeathWhileUnconscious}) then {
      _damageFinal = 0.89;
    };
  } else {
    if (_damageFinal > 0.9 and !(isNil "AGM_Medical_PreventInstaDeath") and {AGM_Medical_PreventInstaDeath}) then {
      _damageFinal = 0.89;
    };
  };
