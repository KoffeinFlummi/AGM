...
if !(call AGM_Goggles_fnc_ExternalCamera) then {
	if (player call AGM_Goggles_fnc_isGogglesVisible) then {
		100 cutRsc["RscAGM_GogglesEffects", "PLAIN",2,false];
		(uiNamespace getVariable ["AGM_Goggles_DisplayEffects", displayNull] displayCtrl 10662) ctrlSetText format["AGM_Goggles\textures\fx\dust\%1.paa", GETDUSTT(DAMOUNT) + 1];
		SETDUST(DAMOUNT,CLAMP(GETDUSTT(DAMOUNT)+1,0,1));
		SETDUST(DBULLETS,0);
	}
	else {
		if (player getVariable ["EyesDamaged", false]) exitWith {SETDUST(DACTIVE,false);SETDUST(DBULLETS,0);SETDUST(DAMOUNT,0);};
		...
		AGM_Goggles_DustHandler = [3] spawn {
			...
			while {_loop} do {
				...
				if(_timeToSleep >= (time - 2.5)) then {
					...
					private "_amount";
					_amount = 1 - (GETDUSTT(DAMOUNT) * 0.125);
					if !(player getVariable ["EyesDamaged", false]) then {
						AGM_Goggles_PostProcessEyes ppEffectAdjust[1, 1, 0, [0,0,0,0], [_amount,_amount,_amount,_amount],[1,1,1,0]];
						AGM_Goggles_PostProcessEyes ppEffectCommit 1;
						sleep 1;
					};
