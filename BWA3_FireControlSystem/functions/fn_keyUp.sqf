...
// MOVING TARGETS
_movingAzimuth = 0;
if (time - BWA3_FCSTime > 1 and BWA3_FCSTime != -1) then {
  ...
  _velocityTarget = [
    ((_posTarget select 0) - (BWA3_FCSPosition select 0)) / (time - BWA3_FCSTime),
    ((_posTarget select 1) - (BWA3_FCSPosition select 1)) / (time - BWA3_FCSTime),
    ((_posTarget select 2) - (BWA3_FCSPosition select 2)) / (time - BWA3_FCSTime)
  ];
