...
private ["_config", "_count", "_array", "_index", "_number", "_hash"];
...
for "_a" from 0 to (_count - 1) do {
	{
		_index = _forEachIndex % 16;
		_number = _array select _index;
		_number = (_number + _x) % 16;
		_array set [_index, _number];
	} forEach (toArray str (_config select _a));
};

_hash = "";
{
	_number = _x call AGM_Core_fnc_toHex;
	_hash = _hash + _number;
} forEach _array;
_hash
