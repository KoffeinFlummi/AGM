...
_this spawn {
  ...
  if ("agm_nametags" in actiavedAddons) then {
    [[[player,_text],{(_this select 0) setVariable ["AGM_currentPBAction",(_this select 1),true]}],"BIS_fnc_spawn",true] call BIS_fnc_MP;
  };
  ...
  closeDialog 0;
  if ("agm_nametags" in actiavedAddons) then {
    [[[player],{(_this select 0) setVariable ["AGM_currentPBAction","",true]}],"BIS_fnc_spawn",true] call BIS_fnc_MP;
  };
