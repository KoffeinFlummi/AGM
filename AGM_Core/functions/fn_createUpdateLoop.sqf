 ...
 * 5: The code that is executed after the loop ends. (Code or String)
 * 
 * Return value:
 * Nothing
 */

private ["_scriptName", "_initCode", "_period", "_updateConditions", "_updateFunctions", "_exitCode", "_header", "_code", "_variable", "_condition", "_function"];
...

{
	...
	_header = _header + format ["private '%1';", _variable];
	_code = _code + format ["if (%1 != %2) then {%1 = %2; [%1] call {%3}};", _variable, _condition, _function];
} forEach _updateFunctions;

_code = format ["%1 %2}; %3};", _header, _loop, [_exitCode] call AGM_Core_fnc_codeToString];
