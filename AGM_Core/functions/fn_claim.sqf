...
if (_lockTarget) then {
  if (!isNull _unit) then {
    [_target, "{_locked = locked _this; _this setVariable ['AGM_lockStatus', _locked]; _this lock 2}", _target] call AGM_Core_fnc_execRemoteFnc;
  } else {
    [_target, "{_this lock (_this getVariable ['AGM_lockStatus', locked _this])}", _target] call AGM_Core_fnc_execRemoteFnc;
  };
};
