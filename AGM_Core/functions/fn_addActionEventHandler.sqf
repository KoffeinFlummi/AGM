...
if (_actionID == -1) then {
	_addAction = call compile format [
		"[
			...
			true,
			'%1',
			""_actions = (_this getVariable '%2') select 1 select 2; _count = count _actions; _index = 0; _return = false; while {_index < _count && {!_return}} do {_return = _this call ((_actions select _index) select 0); _index = _index + 1}; _return""
		]",
		_action,
