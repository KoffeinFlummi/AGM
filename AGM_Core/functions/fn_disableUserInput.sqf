...
if (_this select 0) then {
    ...
    if (isNil "AGM_Core_disableUserInput_ehid") then {
      AGM_Core_disableUserInput_ehid = findDisplay 46 displayAddEventHandler ["KeyDown", {(_this select 1 > 1) and (inputAction "TeamSwitch" == 0) and (inputAction "CuratorInterface" == 0)}];
    };
  } else {
    if (isNil "AGM_Core_disableUserInput_ehid") then {
